...
async def begin(data):
    ...
    for ctrl in ctrls:   
        TASKS.append(loop.run_in_executor(expool, run_loop_in_process, 'ctrl',ctrl, settings, data, networkGraph))

    for node in nodes: 
        if node['issink']:            
            TASKS.append(loop.run_in_executor(expool, run_loop_in_process, 'sink', node, settings, data))
        else:
            TASKS.append(loop.run_in_executor(expool, run_loop_in_process, 'mote', node, settings, data))
...
def run_loop_in_process(nodetype, node, settings, data, networkGraph=None):    
    async def subprocess_async_work():
        ...
        if nodetype == 'ctrl':
            ...
            if nodetype == 'mote':
                ...
                mote = Mote(net=node['net'], addr=Addr(node['addr']), portSeq=node['port'], batt=node['batt'], pos=node_pos, target=target, cls=SENSORS[node['datatype']], \
                    ctrl=(node["ctrl"].split(':')[0], ct.BASE_CTRL_PORT+int(node["ctrl"].split(':')[1])))
