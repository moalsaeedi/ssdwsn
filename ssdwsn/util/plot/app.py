...
# @app.route('/')
# async def index(request):
#     with open('./ssdwsn/util/plot/templates/index.html') as f:
#         return html(f.read())
...
async def savetopo(sid, data):
    ...
    async with aiofiles.open('outputs/'+str(filename)+'.json', mode='w') as f:
        await f.write(data)

@sio.event
async def loadtopo(sid):
    async with aiofiles.open('outputs/topo.json') as f:
        data = json.loads(await f.read())
        await sio.emit('sr_loadtopo', data)
